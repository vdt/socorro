  ...
  try: # outer try/except: level 0
    try: # mid try/finally: level 1
      ...
      try: # inner try/finally level 2: write gzipped files
        ...
        for aCrash in sdb.execute(databaseCursor, sql):
...
app_name = 'daily_csv'
version = '1.1'
doc = """This app produces a csv file of the current day's crash data"""
#-------------------------------------------------------------------------------
rc = cm.Namespace()
rc.option('day',
...
rc.option('outputPath',
          "file system location to put the 'internal/private' output csv file",
          default='.')
rc.option('publicOutputPath',
          "file system location to put the 'external/public' output csv file",
          default='.')
rc.option('product',
