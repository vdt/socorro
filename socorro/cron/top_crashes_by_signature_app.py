...
class TopCrashesBySignature(object):
  ...
  def __init__(self,configContext):
    ...
    self.logger = configContext.logger
    try:
      ...
      lib_util.reportExceptionAndAbort(self.logger)
    ...
    try:
      self.productVersionRestriction = cron_util.getProductId(configContext.product, configContext.version, cursor, self.logger)
    except:
    ...
    print self.logger
    self.logger.debug('%s %s', self.productVersionRestriction, self.productVersionSqlRestrictionPhrase)
    self.startDate,self.deltaDate,self.endDate = cron_util.getProcessingDates(self.configContext,resultTable,self.productVersionRestriction,cursor,self.logger)
  ...
  def tallyPVpairs(self, rows, columns, startDateAsCompactString, previousDateAsCompactString, summaryCrashes, idCache):
    ...
    for r in rows:
      ...
      if row.version[-3:] == 'pre':
        self.logger.debug('%s %s', row.version, row.build)
        try:
        ...
        if buildDateAsCompactString != startDateAsCompactString and buildDateAsCompactString != previousDateAsCompactString:
          self.logger.debug('skipping: %s != %s and %s != %s', buildDateAsCompactString, startDateAsCompactString, buildDateAsCompactString, previousDateAsCompactString)
          continue
  ...
  def extractDataForPeriod(self, startTime, endTime, summaryCrashes):
    ...
    idCache = socorro_cia.IdCache(cur,logger=self.logger)
    ...
    if self.debugging:
      self.logger.debug("Collecting data in range[%s,%s) on column %s",startTime,endTime, self.dateColumnName)
    ...
    try:
      ...
      cursor = self.connection.cursor()
      if self.debugging:
        self.logger.debug(cursor.mogrify(sql,startEndData))
      ...
      self.logger.debug("Returning %s items for window [%s,%s)",len(summaryCrashes),startTime,endTime)
      return summaryCrashes # redundantly
    except Exception, x:
      self.logger.warn('Exception during extractDataForPeriod: %s',x)
  ...
  def fixupCrashData(self,crashMap, windowEnd, windowSize):
    ...
    if self.debugging:
      self.logger.debug("Fixup crash data for %s items",len(crashMap))
  ...
  def storeFacts(self, crashData, intervalString):
    ...
    if not crashData or 0 == len(crashData):
      self.logger.warn("%s - No data for interval %s",threading.currentThread().getName(),intervalString)
      return 0
    # else
    if self.debugging:
      self.logger.debug('Storing %s rows into table %s at %s',len(crashData),resultTable,intervalString)
    sql = """INSERT INTO %s
  ...
  def processDateInterval(self,**kwargs):
    ...
    cursor = self.connection.cursor()
    startWindow,deltaWindow,endWindow = cron_util.getProcessingWindow(self.configContext,resultTable,self.productVersionRestriction,cursor,self.logger,**kwargs)
    startWindow = self.startDate
    try:
      fullCount = 0
      while startWindow + deltaWindow <= self.endDate:
        self.logger.debug("%s - Processing with interval from %s, size=%s)",threading.currentThread().getName(),startWindow,deltaWindow)
...
def main(config):
    import time
    before = time.time()
    tc = TopCrashesBySignature(config)
    count = tc.processDateInterval()
    config.logger.info("Successfully processed %s items in %3.2f seconds",
                       count,
                       time.time()-before)
...
#-------------------------------------------------------------------------------
app_name = 'top_crashes_by_signature'
version = '1.5'
doc = """This cron app will keep the TCBS table updated"""
#-------------------------------------------------------------------------------
rc = cm.Namespace()
rc.option('startWindow',
          'The start of the single aggregation window (YYYY-MM-DD [hh:mm:ss])',
          default=None,
          from_string_converter=cm.datetime_converter)
rc.option('deltaWindow',
          'The length of the single aggregation window  ([dd:]hh:mm:ss)',
          default=None,
          from_string_converter=cm.timedelta_converter)
rc.option('defaultDeltaWindow',
          'The length of the single aggregation window  ([dd:]hh:mm:ss)',
          default='00:12:00',
          from_string_converter=cm.timedelta_converter)
rc.option('endWindow',
          'The end of the single aggregation window (YYYY-MM-DD [hh:mm:ss])',
          default=None,
          from_string_converter=cm.datetime_converter)
rc.option('startDate',
          doc='The start of the overall/outer aggregation window (YYYY-MM-DD '
              '[hh:mm])',
          default=None,
          from_string_converter=cm.datetime_converter)
rc.option('deltaDate',
          'The length of the overall/outer aggregation window  ([dd:]hh:mm:ss)',
          default=None,
          from_string_converter=cm.timedelta_converter)
rc.option('initialDeltaDate',
          'The length of the overall/outer aggregation window  ([dd:]hh:mm:ss)',
          default='96:00:00',
          from_string_converter=cm.timedelta_converter)
rc.option('endDate',
          doc='The end of the overall/outer aggregation window (YYYY-MM-DD '
              '[hh:mm:ss])',
          default=None,
          from_string_converter=cm.datetime_converter)
rc.option('debug',
...
rc.option('product',
          doc='the name of the product to restrict to (leave blank for all)',
          default='Firefox')
rc.option('version',
          doc='the name of the version to restrict to (leave blank for all)',
          default='')
