...
def getProcessingDates(configContext, tableName, productVersionRestriction, cursor, logger, **kwargs):
  return (datetime.datetime(2011,4,1), datetime.timedelta(hours=4), datetime.datetime(2001,5,17))

def getProcessingDates_org(configContext, tableName, productVersionRestriction, cursor, logger, **kwargs):
  """
  ...
  if startDate:
    ...
    startDate = cm.datetime_converter(startDate)
  deltaDate = config.get('deltaDate')
  if type(deltaDate) is str:
    deltaDate = cm.timedelta_converter(deltaDate)
  endDate = config.get('endDate')
  if endDate:
    endDate = "%s"%(endDate)
    endDate = cm.datetime_converter(endDate)
...
def getProcessingWindow_org(configContext,tableName, productVersionRestriction,cursor,logger, **kwargs):
  """
  ...
  if type(startWindow) is str:
    startWindow = cm.datetime_converter(startWindow)
  deltaWindow = config.get('deltaWindow')
  if type(deltaWindow) is str:
    deltaWindow = cm.timedelta_converter(deltaWindow)
  endWindow = config.get('endWindow')
  if type(endWindow) is str:
    endWindow = cm.datetime_converter(endWindow)
  processingDay = config.get('processingDay')
  if type(processingDay) is str:
    processingDay = cm.datetime_converter(processingDay)
  try:
