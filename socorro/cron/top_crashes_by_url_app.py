...
class TopCrashesByUrl(object):
  ...
  def fixupContextByProcessingDay(self,context):
    ...
    if pday:
      ...
      pday = cm.datetime_converter(pday)
...
import socorro.lib.config_manager as cm
#-------------------------------------------------------------------------------
app_name = 'top_crashes_by_url'
version = '1.5'
doc = """This cron app will keep the TCBU table updated"""
#-------------------------------------------------------------------------------
rc = cm.Namespace()
...
rc.option('truncateUrlLength',
          doc='The maximum stored length of a URL (leave None for no '
              'truncation)',
          default=None)
rc.option('product',
...
rc.option('version',
          ...
          default='')
rc.option('processingDay',
          doc='One day to process (YYYY-MM-DD)',
          short_form='d',
          default=None,
          from_string_converter=cm.datetime_converter)
