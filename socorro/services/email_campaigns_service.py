...
#=================================================================================================================
class EmailCampaignsService(rstapi.JsonServiceBase):
  """ Hoopsnake API which provides access to email campaigns

  """
  #-----------------------------------------------------------------------------------------------------------------
  def __init__(self, configContext):
    super(EmailCampaignsService, self).__init__(configContext)
    self.database = db.Database(configContext)

  #-----------------------------------------------------------------------------------------------------------------
  # required_config
  #    note: the email Services all use the same configuration.  To reduce
  #    redundency, they've only been defined in the file
  #    .../socorro/services/email_campaign_service.py under the class
  ...
  def load(self, page_number):
    ...
    try:
      ...
      if page_number < total_pages:
        next = EmailCampaignsService.uri.replace('(.*)', str(page_number + 1))
      if page_number > 1:
        previous = EmailCampaignsService.uri.replace('(.*)', str(page_number - 1))
