...
#=================================================================================================================
class Processor(cm.RequiredConfig):
  ...
  _rc.option('processorCheckInFrequency',
             doc='the frequency in seconds for the processor to check in with '
                 'the monitor',
             default='0:05:00',
             from_string_converter=cm.timedelta_converter)
  _rc.option('processorCheckInTime',
             doc='the time after which a processor is considered dead '
                 '(hh:mm:ss)',
             default='00:05:00',
             from_string_converter=cm.timedelta_converter)
  _rc.option('jsonFileSuffix',
             doc='the suffix used to identify a json file',
             default='.json')
  _rc.option('dumpFileSuffix',
             doc='the suffix used to identify a dump file',
             default='.dump')
  _rc.option('temporaryFileSystemStoragePath',
             doc='a local filesystem path where processor can write dumps '
                 'temporarily for processing',
             default='./')
  _rc.option('numberOfThreads',
             doc='the number of threads to use',
             default=4),
  _rc.option('processorId',
             doc='the id number for the processor (must already exist in '
                 'database) (0 for create new Id, "auto" for autodetection)',
             default='auto')
  _rc.option('processorLoopTime',
             doc='the time to wait between attempts to get jobs (HHH:MM:SS)',
             default='0:00:06',
             from_string_converter=cm.timedelta_converter)
  _rc.option('checkForPriorityFrequency',
             doc='the time bewteen checks for priority jobs (HHH:MM:SS)',
             default='0:01:00',
             from_string_converter=cm.timedelta_converter)
  _rc.option('checkForPriorityFrequency',
             doc='the frequency for the processor to check in with the monitor',
             default='0:05:00',
             from_string_converter=cm.timedelta_converter)
  _rc.option('batchJobLimit',
             doc='the number of jobs to pull in a time',
             default=10000),
  _rc.option('collectAddon',
             doc="if true, parse and collect information about addons from the "
                 "json file; if false, don't",
             default=True,
             from_string_converter=cm.boolean_converter)
  _rc.option('collectCrashProcess',
             doc="if true, parse and collect information about out of process "
                 "crashes; if false, don't",
             default=True,
             from_string_converter=cm.boolean_converter)
  ...
  def __init__ (self, config, sdb=sdb, cstore=cstore, signal=signal,
    ...
    self.crashStorePool = cstore.CrashStoragePool(config, config.storageClass)
    ...
    try:
      ...
      try:
        ...
        if 'auto' == self.config.processorId:
          ...
          raise cm.OptionError("%s is not a valid option for processorId" %
      ...
      if requestedId == 'auto':  # take over for an existing processor
        ...
        try:
          ...
          raise cm.OptionError("ProcessorId %s is not in processors table or "
  ...
  def processJob (self, jobTuple):
    ...
    try:
      ...
      try:
        dumpfilePathname = threadLocalCrashStorage.get_raw_dump_path(jobUuid,
