...
import socorro.lib.config_manager as configurationManager
import socorro.lib.util
import socorro.lib.psycopghelper as psy
import socorro.cron.bugzilla_app as bug
...
def setup_module():
  ...
  if me:
    return
  me = Me()
  me.config = configurationManager.new_configuration(configurationModule=testConfig, applicationName='Testing bugzilla')
...
class TestBugzilla(unittest.TestCase):
  def setUp(self):
    global me
    self.config = configurationManager.new_configuration(configurationModule = testConfig, applicationName='Testing bugzilla')
    ...
    #self.testConfig = configurationManager.Config([('t','testPath', True, './TEST-BUGZILLA', ''),
                                                  ...
                                                  #('', 'daysIntoPast', True, 0),])
    self.testConfig = socorro.lib.util.DotDict({'testPath': './TEST-BUGZILLA',
                                                'testFileName': 'lastrun.pickle',
                                                'daysIntoPast': 0})
