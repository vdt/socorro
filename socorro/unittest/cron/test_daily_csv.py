...
import socorro.lib.config_manager as configManager
...
import socorro.cron.daily_csv_app as dcsv
...
class TestDailyCsv(unittest.TestCase):
  def setUp(self):
    self.outDir = os.path.join(os.path.split(__file__)[0],'Outdir')
    self.config = configManager.new_configuration(configurationModule=commonconfig, applicationName='Test DailyCsv')
  ...
  def testDailyCsvDump_withPublic(self):
    logger.clear()
    self.config['publicOutputPath'] = self.outDir
    self.config['outputPath'] = self.outDir
    self.config['day'] = dt.date(2010,12,25)
  ...
  def testWriteRowToInternalAndExternalFiles(self):
    ...
    for d in data:
      dcsv.writeRowToInternalAndExternalFiles(iF,pF,d)
